...
async def compute_ennemy_position(self):
    ...
    def get_angle_ennemy(self):
        return math.degrees(math.atan((self.arena.ennemy_position.x - self.rolling_basis.odometrie.x)/(self.arena.ennemy_position.y - self.rolling_basis.odometrie.y))-self.rolling_basis.odometrie.theta)
    ...
    if self.anticollison_mode != AntiCollisionMode.DISABLED and self.arena.ennemy_position is not None:
        if self.rolling_basis.odometrie.distance(self.arena.ennemy_position) < CONFIG.STOP_TRESHOLD:

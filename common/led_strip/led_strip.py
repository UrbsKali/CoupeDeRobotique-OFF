...
class LEDStrip:
    def __init__(self, num_leds, pin, freq, brightness):
        self._strip = PixelStrip(num_leds, pin, freq, brightness)
        self._strip.begin()
    ...
    def is_ready(self, state=True):
        self.set_color_single(
            1, Colors.GREEN if state else Colors.RED
        )

    def set_jack(self, state):
        self.set_color_single(
            2, Colors.GREEN if state else Colors.RED
        )

    def set_team(self, team):
        self.set_color_single(
            3, Colors.YELLOW if team == "y" else Colors.BLUE
        )

    def acs_state(self, state):
        self.set_color_single(
            4, Colors.RED if state else Colors.GREEN
        )
